...
class Transformer extends Transform {
  ...
  _transform (sourceRow, encoding, callback) {
    ...
    const filterFns = Array.isArray(this.filterFn) ? this.filterFn : [this.filterFn]

    let filter = true

    filterFns.forEach(fn => {
      if (!fn(outputValues)) filter = false
    })
    ...
    const transformFns = Array.isArray(this.transformerFn) ? this.transformerFn : [this.transformerFn]

    let transformedValues = outputValues

    transformFns.forEach(fn => {
      return new Promise((resolve, reject) => {
        fn(transformedValues, (err, values) => {
    ...
    callback(null, csvEncoder(transformedValues))
